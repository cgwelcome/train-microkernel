...
void basic_track_setup() {
    ...
    switch (track_name) {
        case 'A':
            ...
            train1_node = track_find_node_by_name(&singleton_track, NODE_TRACK_A);
            break;
        case 'B':
            TrainInitTrack(train_tid, TRAIN_TRACK_B);
            train1_node = track_find_node_by_name(&singleton_track, NODE_TRACK_B);
            break;
    }
}
...
void basic_menu(TestCase *suite, uint32_t size) {
    ...
    for (;;) {
        ...
        if ('0' <= option && option <= '0'+size-1) {
            ...
            TrainMove(train_tid, train1_id, train1_speed, train1_node, 0);
        } else if (option == 'r') {
            basic_setup();
        } else if (option == 'q') {
            break;
